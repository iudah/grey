...
void grey_sys_render_draw(EcsRegistry reg) {
  ...
  for (u32 i = 0; i < n_entities;) {
    ...
    if (has_sprite||has_render) {
      ...
      DrawTexturePro(spr->texture, spr->src, (Rectangle){pos->x, pos->y, spr->dest.width,spr->dest.height}, (Vector2){0,0}, 0,spr->tint);
      }
      
      if(has_render){
      RenderComponent *ren = ecs_get_render(reg, e);
      
      DrawRectangle(pos->x, pos->y, ren->width, ren->height, Fade(ren->color, 0.05f));
      }
